...
namespace Fifth_of_July
{
    class Program
    {
        ...
        private static void load_video() {
            ...
            using (TransactionScope transactionScope = new TransactionScope())
            {
                ...
                using (SqlCommand command = new SqlCommand())
                {
                    command.Connection = sqlConnection;
                    command.CommandType = System.Data.CommandType.Text;
                    command.CommandText = String.Format("SELECT FileData.PathName() As Path, FileData FROM {0} WHERE description=@video_name", table_name);
                    ...
                    try
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            string transactionContext = Convert.ToBase64String((byte[])reader["FileData"]);

                            Console.WriteLine("Content of {0} :\n\n {1}",video_name,transactionContext);
                        }
                    }
                    catch (Exception e)
                    {
        ...
        private static void show_menu()
        {
            ...
            while (command != "0")
            {
                ...
                Console.WriteLine("(3) Retrieve Video from Database");
